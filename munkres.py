...
class Munkres:
    ...
    def pad_matrix(self, matrix: Matrix, pad_value: int=0) -> Matrix:
        ...
        **Returns**
        ...
        for ri,row in enumerate(new_matrix):
            sorted_rows.append(ri)
            sorted_cols.append(list(range(max_columns-1,-1,-1)))
        sorted_cols = np.argsort(sorted_cols).tolist()
        sorted_rows = np.argsort(sorted_rows).tolist()
        return new_matrix,sorted_rows, sorted_cols
    ...
    def compute(self, cost_matrix: Matrix) -> Sequence[Tuple[int, int]]:
        ...
        **Returns**
        ...
        through the matrix
        """
        self.C, self.sorted_rows, self.sorted_cols = self.pad_matrix(cost_matrix)
        self.n = len(self.C)
    ...
    def __step2(self) -> int:
        ...
        n = self.n
        for ii in range(n):
            i = self.sorted_rows[ii]
            for jj in range(n):
                j = self.sorted_cols[i][jj]
                if (self.C[i][j] == 0) and \
